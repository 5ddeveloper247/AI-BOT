                            ...
                            <tr id="userRow_{{ $payment->id }}">
                                ...
                                <td>
                                    ...
                                    <div
                                        ...
                                        <input style="font-size: 20px;" type="checkbox" {{ $payment->status
                                        ?
                                        'checked' :
                                        '' }} class="form-check-input"
                                        onchange="toggleActive({{ $payment->user->id }})">
                                ...
                                <td>
                                    <div
                                        class="  order-actions form-check form-switch d-flex justify-content-center align-items-center ">
                                        <input style="font-size: 20px;" type="checkbox" {{ $payment->membership->status
                                        ?
                                        'checked' :
                                        '' }} class="form-check-input"
                                        onchange="toggleActive({{ $payment->user->id }})">
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-light btn-sm radius-30 px-4" data-bs-toggle="modal"
                                        ...
                                        onclick="populateModal2('{{ $payment->user->name }}', '{{ $payment->user->email }}', '{{ $payment->user->phone }}', '{{ $payment->user->active }}')">
                                        Details
                                    </button>
                                ...
                                {{-- <td>
                                    <div class="d-flex order-actions" style="justify-content: space-around;">
                                        <button class="btn btn-danger mx-2" data-bs-toggle="modal"
                                            ...
                                            onclick="populateModal('{{ $payment->user->name }}', '{{ $payment->user->email }}', '{{ $payment->user->phone }}',  '{{ $payment->user->active }}')">
                                            <i class="bx bxs-edit"></i>
                                        </button>

                                        <button onclick="deleteUser({{ $payment->user->id }})" class="btn btn-danger">
        ...
        function toggleActive(userId) {
            fetch(`{{ url('/') }}/admin/users/toggle-active/${userId}`, {
                ...
                .then(data => {
                    ...
                    round_success_noti('Saved successfully!.');
        ...
        function saveChanges() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    round_success_noti('Changes saved successfully:', response);
